...
- **Mehanika:** *vitka* stranica (~120 redaka; **tvrda granica ~150**). Razlog je tvrdo ograničenje:
  *Claude pouzdano slijedi tek ~100–150 uputa, a sustavski prompt već troši ~50.* `CLAUDE.md` **i**
...
- **Mehanika:** ključni trik je **vezivanje uz putanju** preko retka `paths:` u zaglavlju datoteke —
  ...
  Inventar: **32 pravila**, od kojih je ~27 vezano uz putanju; samo ~5 je "uvijek uključeno" (bez `paths:`).
...
- **Mehanika:** `SKILL.md` je YAML zaglavlje (`name`, `description`, `argument-hint`) + numerirani
  ...
  pročita vaš zahtjev na običnom jeziku i odabere pravu vještinu). Inventar: **52 vještine.**
...
- **Mehanika:** svaki agent je uski specijalist (gramatika; raspored/prelijevanje; R kôd; sadržaj
  ...
  `tools`/`disallowedTools` (npr. kritičar **samo za čitanje**), `permissionMode`. Inventar: **18 agenata.**
...
- **Mehanika:** **dozvole** odlučuju što Claude smije bez nadzora. **Okidači (hooks)** se aktiviraju na
  događaje (Stop, PreCompact, PostToolUse, …). Inventar: **7 okidača** (bilježenje sesije, obavijest na
...
- **Analogija:** vatrodojava u zgradi. Ključni uvid: *"Pravila žive u kontekstu i mogu se sažeti i
...
2. **Suparnička petlja *(adversarial loop)*** (srce). **Kritičar (deck-critic)** (samo čita — *ne može*
   uređivati) nabraja svaku grešku; **ispravljač (deck-fixer)** (čita i piše) ih popravlja; kritičar
   ...
   - *Zašto radi:* "Kritičar ne može popravljati datoteke, pa nema poticaja umanjivati probleme.
     Ispravljač ne može odobriti sam sebe." → ubija promašaj "izgleda dobro". **To je recenzija.**

## Dva pravila koja održavaju poštenje

- **Prag 80 / 90 / 95.** Svaka datoteka ocijenjena 0–100. **80** = sigurno za commit; **90** = spremno
  za isporuku (PR); **95** = izvrsnost; **ispod 80 = blokirano.** Primjeri odbitaka: prelijevanje
...
*"Git bilježi **što**; dnevnici sesija bilježe **zašto**."* Upareni okidači (`PreCompact` →
...
**Prvo plan** (samo čita; Claude izradi plan, spremi ga, vi odobrite) **→ Način rada izvođača
*(contractor mode)*** (autonomno): IZRADI → PROVJERI → PREGLEDAJ (paralelni specijalizirani agenti) →
POPRAVI (od najgoreg) → PONOVNO PROVJERI → OCIJENI → konvergiralo? petlja ili predaj sažetak.
...
Ušteda u odnosu na sve-na-Opusu: **50–80%**, bez gubitka kvalitete na mehaničkoj razini. Drugi regulator
...
| Ustavno upravljanje | [`../.claude/constitutional-governance.md`](../.claude/constitutional-governance.md) | Članci I–VI (inače opcionalno / dodaje se kasnije) |
...
| Suparnička petlja | `/review-deck` povezuje kritičar → ispravljač → ponovna provjera | srce, u malom |
...
5. Pokrenite `/commit` — gledajte kako **pita prije** `git init` i **prije** commita. *Vi* ostajete glavni.

Taj krug — plan → djeluj → provjeri → pregledaj → vaše odobrenje — *jest* cijeli harness u malom.
